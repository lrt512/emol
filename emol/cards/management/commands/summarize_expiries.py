...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logger.info(f"Expiry summary for {period_name} ({today().strftime(DATE_FORMAT)} to {end_date.strftime(DATE_FORMAT)})")
        ...
        ).select_related('combatant', 'discipline').order_by('date_issued')
        
        # Query for expiring waivers  
        expiring_waivers = Waiver.objects.filter(
            date_signed__lte=today() - timedelta(days=365*7-days_ahead),
            date_signed__gt=today() - timedelta(days=365*7)
        ).select_related('combatant').order_by('date_signed')
        ...
        logger.info(f"📊 SUMMARY: {total_count} total expiries ({card_count} cards, {waiver_count} waivers)")
            ...
            if detailed:
                ...
                for expiry_date in sorted(cards_by_date.keys()):
                    ...
                    for card in cards_by_date[expiry_date]:
                        logger.info(f"      {card.combatant.sca_name} - {card.discipline.name}")
        ...
        if total_count > 0:
            ...
            reminder_count = 0
            reminder_breakdown = defaultdict(lambda: {'total': 0, 'card': 0, 'waiver': 0})
            ...
            if reminder_count > 0:
                logger.info(f"📬 REMINDERS TO SEND: {reminder_count} total reminders during {period_name}")
                for reminder_day in sorted(reminder_breakdown.keys()):
                    ...
                    logger.info(f"   {reminder_day}-day reminders: {total} (waiver: {waiver_count}, card: {card_count})")
            else:
                logger.info(f"📬 REMINDERS TO SEND: No reminders scheduled during {period_name}")
