...
def generate_compose(mode: str, registry: str, version: str, config_dir: str) -> str:
    ...
    if os.path.exists(f"{config_dir}/emol_credentials.json"):
        mount_str = f"      - {config_dir}/emol_credentials.json:/opt/emol/emol_credentials.json:ro"  # noqa: E501
...
def main():
    ...
    parser.add_argument(
        "--version", default="latest", help="Image version tag (default: latest)"
    )
    parser.add_argument(
        "--emol-home", default=os.path.expanduser("~/emol"), help="eMoL home directory"
    )
    parser.add_argument(
        "--config-dir", default="/opt/emol_config", help="Configuration directory"
    )
    ...
    try:
        content = generate_compose(
            args.mode, args.registry, args.version, args.config_dir
        )
